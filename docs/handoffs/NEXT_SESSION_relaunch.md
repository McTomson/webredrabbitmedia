# Naechste Session — relaunch (Stand 2026-07-05 spaet, Session-Ende mitten in der Morph-Praezisions-Runde)

## SOFORT-AUFTRAG DIESER SESSION (Tomson-Mandat 05.07. spaet, woertlich sinngemaess)
Die Morph-Engine nach docs/MORPH_DELTA_ANALYSE.md (PFLICHT-LEKTUERE #1!) auf Original-Niveau bringen. Tomson-Urteil zum jetzigen Stand: ~25% vom Original ("billiger China-Fake") — die Delta-Analyse listet die 8 konkreten Unterschiede und den Nachtrag mit der ENTSCHEIDENDEN Strategie: Formations-Kompositionen NICHT selbst erfinden, sondern die exakten Zielkoordinaten (x,y,rot,scale je Teil, je Szene) aus den extrahierten Original-Lottie-JSONs (~/dev/at-reference-lottie/anim_*.json) parsen und UNSERE 14 Teile auf diese Slots mappen (Tomson-Freigabe als Uebergangsloesung; keine at-Dateien einbinden, nur Messdaten).
ARBEITSWEISE (explizit freigegeben): parallele Agenten MIT BELIEBIGEN LLMs (Opus/Sonnet, muss nicht Fable sein) fuer Parser/Mechanik; Loop + Hooks + Design-Skills; wenn fertig AKTIVER Browser-Vergleich: all-turtles.com UND unsere Seite an mehreren Scroll-Positionen nebeneinander screenshotten und abgleichen; falls localhost nicht reicht, eigenes Vercel-Preview-Deployment (GitHub vorhanden, Branch relaunch deployt schon als Vercel-Branch-Preview!) zum Live-Vergleich nutzen. ERST zurueckmelden, wenn du selbst 100% zufrieden bist — Zwischenstaende nur bei echten Tomson-Entscheidungen.
REFERENZ-MATERIAL (dauerhaft gesichert): ~/dev/at-reference-videos/ (original-allturtles-scroll-63s.mov + unsere-version-2026-07-05.mov + frames-original/ + frames-unsere/, je 2fps JPGs) und ~/dev/at-reference-lottie/ (9 Original-JSONs, NUR Studium/Datenquelle, NIE committen).
KONKRETE SCHRITTE: (1) Lottie-Parser (Python, analog zur Keyframe-Statistik von 04.07.): pro Szenen-Precomp den statischen Halte-Frame -> Teil-Liste normiert 1920x1080 -> als Daten in lib/relaunch/morph/formations.ts. (2) ScenesMorph: 150-200 Teile-Pool, Z-Tiefe (Groesse), Seiten-ALTERNIERUNG (Zahnraeder links/Birne rechts/Dokument links/Chart rechts/Kopf links, Text gegenueber), Morph Form->Form OHNE leeren Screen, Rand-Zu-/Abgang. (3) Hero-Zerfall fuellt ganzen Screen (Vermehrung sichtbar). (4) FooterReassembly: Teile REGNEN von oben. (5) Timing gegen beide Videos. (6) OFFENE TOMSON-ENTSCHEIDUNG: Font (Fraunces behalten vs. Heldane kaufen ~50-200 EUR) — Vergleich visuell vorlegen.
