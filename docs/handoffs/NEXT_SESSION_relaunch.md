# Naechste Session — relaunch (Stand 2026-07-04 spaet: MORPH-ENGINE V2 FERTIG, wartet auf Tomson-Video-Gegenprobe)

## STAND: Der Sofort-Auftrag der Vorsession ist UMGESETZT (Engine v2, Commits 110ab35..)
- HomeMorph = EINE durchgehende Buehne (Hero + 5 Szenen, Pool 175 Teile, nie leerer Screen). HeroMorph/ScenesMorph/formations.ts GELOESCHT.
- Formations-Slots = vermessene Original-Kompositionen (tools/parse-at-lottie.py -> lib/relaunch/morph/at-scenes.ts + at-formations.json). Seiten-Alternierung L-R-L-R-L aus den Daten, Statements auf der Gegenseite, Motiv-Semantik (Dokument=Content, Chart=Dashboard, Kopf=KI; decisions-log).
- Zerfall: Vollbild-Raster ~5vw Teilgroesse (Live-Messung all-turtles 1440x900!), radiale Burst-Zuordnung, Klon-Vermehrung sichtbar, Rest im Aussenring (Rand-Zugang). Footer dunkel, Teile regnen von oben, Wortmarke gross. Lenis lerp 0.075.
- QA durchgefahren: Szenen-Stills gegen Original-Frames + LIVE all-turtles.com im selben Viewport (Birne deckungsgleich), Uebergaenge luecklos, 61 FPS, Konsole sauber, Build gruen, Mobile-Pass (Formation oben/Text unten). Vergleichs-Video: ~/dev/at-reference-videos/unsere-version-v2-morph-engine-2026-07-04.mp4
- OFFEN: (1) Tomson-Video-Gegenprobe der v2 (sein Urteil ist das Gate; letzte Feinheiten Timing/Traegheit danach). (2) Vercel-Branch-Preview hat SSO-Protection (302) — Thomas eingeloggt oeffnen: https://webredrabbitmedia-git-relaunch-toms-projects-17d37f0b.vercel.app/relaunch-preview (3) Offene Tomson-Fragen unten (Case-Panels, almtal-Domain, Methode).

## LESSONS dieser Session (fuer die naechste)
- Lottie-fromX/Y sind Einflug-Startpunkte von AUSSEN (31/129 im Bild) — als Zerfall-Muster unbrauchbar; Zerfall = eigenes gejittertes Raster.
- CSS-transform scale>1 rastert SVG-Teile matschig -> Elemente in Maximal-Groesse rendern, nur downscalen.
- Lenis kaempft gegen window.scrollTo (Race) -> fuer Scroll-Automation Wheel-Events dispatchen; Stills: 2x scrollTo + warten.
- `npm run build` zerschiesst dem laufenden next dev das .next -> dev danach neu starten.
- agent-browser: Recording mit `record restart <file> <url>` startet frischen Kontext; Scroll-Restore beachten (scrollRestoration manual + doppelt scrollTo(0,0)).
