...
**Nach Freigabe headless (Stand 2026-06-19):** **Podcast UND Video laufen jetzt headless automatisch.**
Podcast ueber `media/generate-podcast.sh` (NotebookLM-MCP + Pool leerer Notebooks in
`content-engine/knowledge/podcast-notebook-pool.json`, da die MCP headless kein Notebook anlegen kann).
**Video ueber `media/generate-video.sh`** (notebooklm-py CLI, Volltext-Quelle via `media/extract-body.ts`
— KRITISCH: Video scheitert mit URL-Quelle, gelingt nur mit Volltext-Paste). Der media-checker erzeugt
Podcast + Video und faehrt `run-media --podcast … --video …` EINMAL (einbetten/YouTube-public/push/Mail/
Marker). Ein Video-Fehler bricht den Podcast-Versand NIE ab; nur wenn beide fehlen, Fallback auf die
Browser-Notification. **Noch Browser-gebunden:** Gemini-Hero/Kontextbilder (Codex tot bis ~14.07),
Substack-Draft. Podcast-Pool bei niedrigem Vorrat im Browser nachfuellen (t.uhlir@immo.red). Den ganzen
Flow AUTOMATISCH durchziehen, NICHT pro Schritt fragen (Thomas 16.06): YouTube=public auto, Substack=Draft.
Nur bei echtem Fehler melden.
