...
// The page renders the title from frontmatter as the single <h1> (BlogPostClient). Article bodies
// generated by the pipeline start with a duplicate `# Title` line, which the MDX renderer turned
// into a SECOND <h1> — an SEO defect (one H1 per page) AND a visual duplication (title shown twice).
// Strip a leading top-level `# ` heading (the redundant title) once, centrally, for all articles.
export function stripLeadingTitleH1(content: string): string {
    // Skip leading blank lines, then drop the first line if it is a single-`#` (h1) heading.
    const m = content.match(/^(\s*)(#\s+[^\n]*\n?)/);
    if (m && /^#\s+/.test(m[2]) && !/^##/.test(m[2])) {
        return content.slice(m[0].length).replace(/^\s*\n/, '');
    }
    return content;
}
...
export async function getPostBySlug(slug: string): Promise<BlogPost | null> {
    try {
        ...
        const content = stripLeadingTitleH1(rawContent);
