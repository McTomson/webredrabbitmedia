...
- **PREISE-SEITE UMGESTELLT + COCKPIT (Thomas 30.07.2026):** Paketpreise auf der Relaunch-Preisseite jetzt Starter **1.250** / Business **2.850** / Premium **ab 4.900** (ersetzt 950/2.900; Premium unveraendert, kein neuer Wert genannt). Neues Inklusiv-Merkmal in ALLEN Paketen: **"Dein Cockpit"** = persoenliches Dashboard, sammelt Infos zum eigenen Auftritt aus Google und anderen Quellen an einem Ort (Begriffswahl "Cockpit" statt "Kommandozentrale"/"Copilot": kurz, etabliert, keine Microsoft-Verwechslung — Fable-Entscheidung auf Thomas' "such dir aus"). Talos + KI-Agenten = COMING SOON (Badge "Bald verfuegbar" an der Talos-Talente-Sektion; zum Start gibt es nur das Cockpit mit gesammelten Infos). OFFEN weiterhin: brand/pricing.md + lib/config.ts PRICING (Live-Site) angleichen.
- **BLOG-ARTIKEL-PREISFRAGEN GEKLAERT (Thomas 31.07.2026, Commit 6fe2814):** website-10-seiten-kosten.mdx (10 Seiten = Business-Umfang) 1.990 -> **2.850**; herold-webseite-vs-agentur-vergleich.mdx (Boutique-Vergleich = Business-Umfang) 1.990 -> **2.850**, inkl. HeroldComparisonTable.tsx die noch den AELTEREN 790-Preis hatte (vom letzten Update uebersehen, da .tsx statt .mdx) und deren Ersparnis-Ableitung: bei 2.850 gegenueber Herolds guenstigstem Tarif (65 EUR/Monat) war die 3-Jahres-Ersparnis rechnerisch negativ (-510 EUR) -- Horizont auf **4 Jahre** gestreckt (Thomas-Entscheidung), dort gewinnt der Kauf wieder gegenueber der gesamten Herold-Range; was-kostet-eine-website.mdx ROI-Beispiel (generisches Einsteiger-Beispiel) 2.500 -> **1.250** (Starter-Tier). Talos-Hand-Fix (Punkt C aus dem 25.07.-Grill, Commit 073b89f) im selben Zug erledigt: Begruessung nutzt jetzt die korrigierte "other"-Armkette statt der alten "primary"-Seite, an allen drei Stellen (Hero-Wave, Stations-Wave, Doppelklick-Zyklus).
- **BEGRIFF "COPILOT" STATT "COCKPIT" (Thomas 01.08.2026, kippt die 30.07.-Entscheidung):** Thomas hat auf direkte Rueckfrage (Copilot vs. Cockpit, inkl. Hinweis auf die 30.07.-Microsoft-Verwechslungs-Begruendung und die Live-"Dein Cockpit"-Nutzung) entschieden: **"Copilot ueberall"**. Der Dashboard-Helfer auf /leistungen/website heisst jetzt Copilot (Talos = der Copilot, "bei jeder Website dabei", zeigt Status/Besucher/Herkunft/Klicks, Infos aus Google Search/Heatmap/Google Tag). Die 30.07.-"Cockpit"-Entscheidung ist damit ueberholt und NICHT wieder aufzurollen. OFFEN (eigene Runde): "Cockpit" -> "Copilot" konsistent nachziehen auf der /preise-Seite (Merkmal "Dein Cockpit" in allen Paketen) und ueberall wo "Cockpit" sonst steht.

## 2026-08-08 — Preise: buchbare Zusatz-Module RAUS, alle Preise "ab", 1-2 Vorschlaege, 40 % Anzahlung (Thomas)
- **Keine buchbaren Zusatz-/Modul-Pakete mehr:** Wir bieten aktuell KEINE zum Selbst-Zusammenklicken buchbaren Talos-Module an. Auf /preise entfernt: der Mehrwert-Rechner (MehrwertRechner.tsx, "Talos-Team zusammenklicken", Monatspreise, "dazubuchen/nachbuchbar") und die Faehigkeits-Fahrt der Talos-Talente (Einzel-Module + #rechner-Links). ERSETZT insoweit die Darstellungs-Vorgabe vom 24.07. (Mehrwert-Rechner + buchbare Talos-Talente auf der Preisseite). Die Modul-PREISE selbst (24.07.) bleiben als interne Kalkulation gueltig; sie werden nur nicht mehr als Selbst-Baukasten auf der Seite angeboten. Zusatzleistungen laufen ueber "auf Anfrage". Der 3D-Talos + das Positionierungs-Panel ("135 Jahre Erfahrung") BLEIBEN auf /preise (Talos-Station in TalosTalenteFahrt.tsx erhalten, nur die buchbaren Inhalte raus).
- **Alle Paketpreise sind Startpreise ("ab"):** Starter ab 1.250, Business ab 2.850, Premium ab 4.900 (vorher waren Starter/Business Fixpreise OHNE "ab"). Begruendung sichtbar: je nach Anforderung/Extras kann sich der Umfang und damit der Preis verschieben; das genaue, verbindliche Angebot bekommt der Kunde bei der Auftragserteilung.
- **1-2 grafische Vorschlaege statt "ein Entwurf":** Im Zuge der (unverbindlichen) Anfrage bekommt der Kunde ohne Vorkasse 1-2 grafische Vorschlaege, wohin das Website-Design gehen kann. Gefaellt die Richtung -> Auftragserteilung. Site-weit nachgezogen (Preise, ueber-uns, Kontakt/Lead, Leistungen, Home-Sphere, Talos-Sektion, Demo-Heroes, FAQ). ERSETZT/konkretisiert die fruehere "Design-Vorschlag/Entwurf ohne Vorkasse"-Formulierung. Wort "gratis" bleibt verboten -> "ohne Vorkasse / ohne Aufpreis / kostenlos".
