...
- **Ursache (Zeitzone Producer vs. Consumer).** `app/api/approve/route.ts` laeuft auf Vercel (UTC) und schrieb `requestedAt = new Date().toISOString().slice(0,10)` = **UTC-Datum**. Der media-checker laeuft auf dem Mac und vergleicht gegen `date +%F` = **lokales** Datum (Europe/Vienna). Bei Freigabe zwischen 00:00 und 02:00 CEST ist UTC noch der Vortag -> Marker traegt den Vortag -> `requested == TODAY` schlaegt nie an -> der Artikel faellt jede Nacht in diesem 2h-Fenster durch und bekommt nie Medien. Heute um 00:35 freigegeben -> genau getroffen. Fix: `new Intl.DateTimeFormat('en-CA', { timeZone: 'Europe/Vienna' }).format(new Date())` -> deckungsgleich mit dem Checker. **Lehre:** Datum/Zeit NIE in einer Zeitzone schreiben und in einer anderen vergleichen; UTC `toISOString()` auf einem UTC-Server ergibt fuer „heute lokal" das falsche Datum.
- **Folge-Symptom — voreiliger Mac-Fallback.** Beim manuellen Nachziehen war ein ctx-Bild flaky (der bekannte /glic-Hijack, viele Retries) -> der VPS-Render dauerte ~19 Min. Das alte 12-Min-Poll-Limit in `render_images_via_vps` loeste faelschlich „VPS-Render Timeout" aus und fiel auf LOKALES Rendern zurueck (PID-stack auf dem Mac, Last sprang auf 8) — genau die Ueberlast, die der VPS-Umzug beseitigen sollte, obwohl der VPS in Wahrheit 3/4 Bilder schon fertig hatte. Fix: Poll 72->150 Iterationen (~25 Min) + Frueh-Abbruch wenn der VPS-Render-Prozess stirbt. Der VPS-Render belastet den Mac nicht, langes Warten ist gratis. **Diagnose-Reflex bei „VPS-Render Timeout":** ZUERST `ssh ionos "tail /home/redrabbit/logs/genimg.out"` + `pgrep generate-images` auf dem VPS pruefen — laeuft er noch, NICHT lokal fallbacken; lokalen `generate-images-gemini.sh` killen (Eltern zuerst), VPS fertig rendern lassen, PNGs ziehen, `apply-images-browser.ts` einbetten.
- **Manuelles Nachziehen (Rezept, 0 Extra-Kosten):** Marker-Datum korrigieren ODER warten; dann VPS-PNGs holen (`tar` ueber ssh) -> `apply-images-browser.ts <slug>` (einbetten) -> `generate-podcast-cli.sh <slug>` -> `generate-video.sh <slug>` -> `run-media.ts --slug <slug> --no-images --podcast … --video …` (YouTube public + push + Mail + Marker-Loeschung). Der `--no-images`-Flag verhindert Codex-Neurender.
- **Bestaetigt nebenbei:** Rotations-Fix von gestern lief produktiv (erster echter Artikel = gelber Hero, `colorCursor` committet auf 1 -> naechster gruen), und die Realismus-Regel lieferte realistische Kontextbilder (Beraterin/Plugin-Updates/Kundin am Handy/Wartungsgespraech), kein surreales Stillleben.

## 2026-06-28 — Hero-Farbrotation kaputt (2 Ursachen) + Bilder zu surreal (Art-Director)

Thomas: (a) Hero-Hintergrund immer blau/turquoise statt zu rotieren, (b) Kontextbilder wirken surreal/unlogisch (echtes Beispiel: Lego + Multitool + Holzloeffel auf Betonboden). Beides gefixt in `image.ts` + `run-media.ts` (commit `c3ee35e`), per VPS-Render in echten Pixeln verifiziert.

- **Rotation Ursache 1 — gekappter Zaehler.** `pickHeroColorIndex()` war `readMotifLog().heroes.length % N`. Aber `recordMotif` kappt `heroes` auf `.slice(0,12)` -> sobald ≥12 Artikel existieren steht `heroes.length` FUER IMMER auf 12 -> `12 % 4 = 0` -> konstant Index 0. Fix: dedizierter UNBEGRENZTER `colorCursor` im Motif-Log, advance-on-pick (lesen -> +1 persistieren -> alten Wert % N zurueck), idempotent weil das Ergebnis in `gemini-meta.json` gepinnt wird (Reruns advancen nicht doppelt).
- **Rotation Ursache 2 (versteckt) — Cursor wurde nie committet.** Der neue VPS-Flow `run-media.ts` stagte nur `content/blog/*.mdx, public/audio, public/images/blog, public/videos` — NICHT `recent-image-motifs.json`. `build-image-plan.ts` advanced den Cursor zwar lokal beim Planen, aber der naechste Lauf macht `git reset --hard origin/main` -> Fortschritt weg -> jeder Artikel startet vom selben committeten Wert. Selbst der korrekte Cursor haette ohne diesen Commit nicht rotiert. Fix: `recent-image-motifs.json` in den `git add` von run-media.ts aufgenommen. **Lehre:** jeder Zustand, der zwischen Laeufen ueberleben muss UND vor `git reset --hard` steht, MUSS committet werden — sonst ist er effektiv ephemer.
- **Palette propagiert nur ueber git, nicht ueber den Plan.** `generate-images-gemini.sh:100` ruft `build-image-plan.ts` AUF DEM VPS auf. Der Bild-KONZEPT-Teil kommt aus der mitgelieferten `plan.json` (Mac), aber der Farb-GRADIENT wird aus der VPS-eigenen `image.ts` abgeleitet. Folge: nach einer Palette-Aenderung rendert der VPS so lange die ALTE Farbe, bis `image.ts` auf `origin/main` ist und der VPS sich darauf resettet. Verifiziert: vor Push rendert colorIdx 0 turquoise (alte Palette), nach Push baut der VPS "yellow #F7C948". Schnelltest ohne Render: auf dem VPS `build-image-plan.ts` laufen lassen und das `images[0].prompt`-Gradientwort pruefen.
- **Bildinhalt — Realismus-Pflicht im Art-Director.** Die alte Prompt-Regel erzwang aktiv Archetyp "(a) Konzept/Still-Life (Objekte, die das Thema symbolisieren, keine Person)" -> genau die surrealen Objekt-Arrangements. Fix: Still-Life-Archetyp entfernt, harte REALISMUS-PFLICHT ergaenzt (reale, alltaegliche, sofort verstaendliche Szene; abstrakte Metaphern/drapierte Gegenstaende STRENG verboten; lieber konkrete Szene mit Menschen) — in `buildImagePlan`-Prompt UND als Klausel in `BRAND_PHOTO_STYLE` (Guertel+Hosentraeger, greift bei Gemini direkt). Verifiziert am selben Artikel: Lego-Stillleben -> Beratungsgespraech / Entwickler am Monitor / Cafe-Besitzerin.
- **Infografik unberuehrt** (lokale SVG `sketchInfographic`) — Thomas mag den Stil; nicht angefasst.
- **QA-Methodik (0 Zusatzkosten):** Art-Director-Konzepte zuerst als TEXT pruefen (1 claude-Aufruf, kein Render) ist der hoechste-Signal-Test fuer den Realismus-Fix; Render nur zur Pixel-Bestaetigung. Motif-Log bei lokalen Tests IMMER per Backup/Restore schuetzen (pickHeroColorIndex + buildImagePlan schreiben hinein).
