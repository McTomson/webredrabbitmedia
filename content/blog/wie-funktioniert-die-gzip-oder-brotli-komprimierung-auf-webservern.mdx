...
excerpt: >-
  Gzip und Brotli komprimieren HTML, CSS und JavaScript auf dem Server, bevor
  sie beim Besucher ankommen, und sparen dabei oft bis zu 70 Prozent
  Datenvolumen. Wie der Handshake zwischen Browser und Server funktioniert und
  worauf Sie bei Next.js und Cloudflare achten sollten.
featuredSnippetTitle: Wie funktioniert Gzip oder Brotli Komprimierung auf Webservern?
featuredSnippet: >-
  Gzip und Brotli sind Kompressionsverfahren, die Text-Dateien wie HTML, CSS und
  JavaScript auf dem Server verkleinern, oft um bis zu 70 Prozent, bevor sie zum
  Browser geschickt werden. Der Browser meldet per Accept-Encoding-Header,
  welches Verfahren er versteht, der Server wählt eines aus und bestätigt es per
...
tags:
  ...
  - Next.js
featuredImage: >-
  /images/blog/wie-funktioniert-die-gzip-oder-brotli-komprimierung-auf-webservern.png
status: published
chosenHook: 'html & css: bis zu 70% komprimiert?'
aiAssisted: true
sources:
...
keyTakeaways:
  - >-
    Kompression läuft über Content-Negotiation: Der Browser fragt per
    Accept-Encoding an, der Server bestätigt per Content-Encoding, und Vary:
    Accept-Encoding schützt Caches vor falschen Varianten.
  - >-
    Textdateien lassen sich um bis zu 70 Prozent verkleinern, Bilder in gängigen
    Formaten dagegen kaum, weil sie bereits komprimiert sind.
  - >-
    Brotli komprimiert stärker als Gzip, braucht dafür aber mehr Rechenzeit,
    weshalb Gzip bei dynamischen, nicht cachebaren Inhalten oft die bessere Wahl
    bleibt.
  - >-
    Cloudflare wählt das Verfahren tarifabhängig automatisch: Zstandard im
    Free-Plan, Brotli im Pro- und Business-Plan, Gzip im Enterprise-Plan.
  - >-
    Next.js komprimiert mit Gzip von Haus aus, bei einem vorgeschalteten Server
    oder CDN sollte compress: false gesetzt werden, um doppelte Kompression zu
    vermeiden.
conclusionStats:
...
customFAQs:
  - question: Was ist der Unterschied zwischen Gzip und Brotli Komprimierung?
    answer: >-
      Brotli komprimiert Text-Dateien stärker als Gzip, weil es ein modernes
      Kontextmodell nutzt, braucht dafür aber mehr Rechenzeit beim Komprimieren
      selbst. Gzip ist schneller im Erzeugen und deshalb bei dynamischen, nicht
      cachebaren Inhalten oft die praktischere Wahl, während Brotli sich für
      statische, oft wiederverwendete Dateien lohnt.
  - question: Wie aktiviere ich Gzip oder Brotli Komprimierung auf meinem Server?
    answer: >-
      Bei Next.js ist Gzip standardmäßig aktiv, sobald die Anwendung über next
      start läuft. Läuft ein CDN oder ein Server wie Nginx davor, übernimmt
      dieser die Kompression, dann sollte compress: false in der next.config.js
      gesetzt werden, damit nicht doppelt komprimiert wird.
  - question: Welches Kompressionsverfahren nutzt Cloudflare?
    answer: >-
      Cloudflare wählt das Verfahren abhängig vom Tarif automatisch aus. Der
      kostenlose Free-Plan nutzt Zstandard, der Pro- und der Business-Plan
      setzen auf Brotli, und der Enterprise-Plan verwendet Gzip. Welches
      Verfahren tatsächlich ausgeliefert wird, hängt zusätzlich vom
      Accept-Encoding-Header des jeweiligen Besuchers ab.
  - question: Komprimiert Next.js Websites automatisch?
    answer: >-
      Ja, Next.js aktiviert Gzip-Kompression standardmäßig für gerenderte Seiten
      und statische Dateien, sobald die Anwendung über next start oder einen
      eigenen Server läuft. Ein separates Setup ist dafür nicht nötig, außer ein
      vorgeschalteter Dienst komprimiert bereits selbst.
  - question: Lohnt sich Kompression auch für Bilder wie JPEG oder PNG?
    answer: >-
      Nein, gängige Bildformate wie JPEG, PNG oder WebP sind bereits selbst
      komprimiert. Ein zweiter Durchlauf mit Gzip oder Brotli bringt praktisch
      keine zusätzliche Ersparnis und kostet nur Rechenzeit. Kompression auf
      Server-Ebene lohnt sich vor allem bei Text-basierten Dateien wie HTML,
      CSS, JavaScript, JSON oder SVG.
hookCandidates:
...
<SimpleAudioPlayer src="/audio/wie-funktioniert-die-gzip-oder-brotli-komprimierung-auf-webservern-podcast.mp3" title="Podcast: Gzip vs. Brotli: Komprimierung auf Webservern (2026)" />

<VideoEmbed src="/videos/wie-funktioniert-die-gzip-oder-brotli-komprimierung-auf-webservern-video.mp4" poster="/videos/wie-funktioniert-die-gzip-oder-brotli-komprimierung-auf-webservern-poster.jpg" id="sLVhe3cGtu4" title="Video: Gzip vs. Brotli: Komprimierung auf Webservern (2026)" />
...
![Kellner und Gast im Wiener Kaffeehaus stimmen eine Bestellung ab, wie Browser und Server per Accept-Encoding das Kompressionsverfahren aushandeln](/images/blog/wie-funktioniert-die-gzip-oder-brotli-komprimierung-auf-webservern-ctx1-srty8.png)
...
![Gzip oder Brotli?](/images/blog/wie-funktioniert-die-gzip-oder-brotli-komprimierung-auf-webservern-infografik-srtyb.png)
...
![Technikerin geht durch einen Serverraum mit Rechenzentrums-Racks, wo CDNs wie Cloudflare die Kompression automatisch uebernehmen](/images/blog/wie-funktioniert-die-gzip-oder-brotli-komprimierung-auf-webservern-ctx2-srv9n.png)
...
![Mann prueft in den Browser-Entwicklertools die Content-Encoding-Header seiner Website, um Gzip- oder Brotli-Kompression zu kontrollieren](/images/blog/wie-funktioniert-die-gzip-oder-brotli-komprimierung-auf-webservern-ctx3-srv9t.png)
